export const PrivacidadContent = {
    es: {
      title: "Política de Privacidad",
      sections: [
        {
          heading: "1. Información del Responsable del Tratamiento",
          content:
            "El responsable del tratamiento de los datos personales recogidos a través de este sitio web es Artaza Inmobiliaria, una sociedad dedicada a la prestación de servicios inmobiliarios y con sede en Leioa (Bizkaia). Gestionamos tu información de forma responsable y conforme a la normativa vigente en materia de protección de datos. Para cualquier consulta relacionada con tus datos personales o el ejercicio de tus derechos, puedes escribirnos a info@inmoartaza.es.",
        },
        {
          heading: "2. Finalidades del Tratamiento",
          content:
            "Los datos personales facilitados a través del sitio web serán tratados con las siguientes finalidades:\n- Atender sus solicitudes de información, consultas o peticiones.\n- Gestionar la relación comercial y prestar los servicios inmobiliarios ofrecidos.\n- Envío de comunicaciones comerciales, siempre que el usuario haya dado su consentimiento expreso.\n- Cumplimiento de obligaciones legales aplicables.",
        },
        {
          heading: "3. Legitimación para el Tratamiento de sus Datos",
          content:
            "La base legal para el tratamiento de sus datos personales es:\n- Consentimiento del interesado, otorgado al rellenar y enviar formularios en nuestra web.\n- Ejecución de un contrato o precontrato, en caso de que solicite nuestros servicios.\n- Cumplimiento de obligaciones legales aplicables a Artaza Inmobiliaria",
        },
        {
          heading: "4. Plazo de Conservación de los Datos",
          content:
            "Los datos se conservarán durante el tiempo necesario para cumplir con la finalidad para la que se recabaron y para determinar las posibles responsabilidades que se pudieran derivar de dicha finalidad. En el caso de datos para el envío de comunicaciones comerciales, se conservarán mientras el interesado no revoque su consentimiento.",
        },
        {
          heading: "5. Comunicación de Datos a Terceros",
          content:
            "No se cederán datos a terceros, salvo obligación legal o cuando sea necesario para la prestación del servicio contratado (por ejemplo, notarios, registros, o proveedores tecnológicos que actúan como encargados del tratamiento).",
        },
        {
          heading: "6. Derechos del Usuario",
          content:
            "Tiene derecho a:\n- Acceder a sus datos personales.\n- Solicitar la rectificación de los datos inexactos.\n- Solicitar su supresión cuando los datos ya no sean necesarios.\n- Oponerse al tratamiento de sus datos o solicitar la limitación del mismo.\n- Solicitar la portabilidad de sus datos.\n- Retirar el consentimiento en cualquier momento, sin que ello afecte a la licitud del tratamiento previo a su retirada.\n\nPuede ejercer estos derechos dirigiendo una solicitud a la dirección postal o electrónica indicadas en el apartado 1, acompañando copia de su DNI o documento equivalente.\nAsimismo, tiene derecho a presentar una reclamación ante la Agencia Española de Protección de Datos (www.aepd.es) si considera que sus derechos han sido vulnerados.",
        },
        {
          heading: "7. Medidas de Seguridad",
          content:
            "Artaza Inmobiliaria adopta las medidas técnicas y organizativas necesarias para garantizar la seguridad, integridad y confidencialidad de los datos personales y evitar su pérdida, alteración o acceso no autorizado.",
        },
        {
          heading: "8. Modificaciones de la Política de Privacidad",
          content:
            "Nos reservamos el derecho a modificar la presente política para adaptarla a novedades legislativas o jurisprudenciales. En dicho caso, anunciaremos los cambios en esta página con antelación razonable a su puesta en práctica.",
        },
      ],
    },
    en: {
      title: "Privacy Policy",
      sections: [
        {
          heading: "1. Data Controller Information",
          content:
            "The data controller responsible for the processing of personal data collected through this website is Artaza Inmobiliaria, a company dedicated to providing real estate services and based in Leioa (Bizkaia), Spain. We manage your information responsibly and in accordance with current data protection regulations. For any inquiries related to your personal data or to exercise your rights, you can contact us at info@inmoartaza.es.",
        },
        {
          heading: "2. Purposes of Data Processing",
          content:
            "The personal data provided through the website will be processed for the following purposes:\n- Respond to your requests for information, inquiries, or petitions.\n- Manage the business relationship and provide the offered real estate services.\n- Send commercial communications, provided the user has given express consent.\n- Fulfill applicable legal obligations.",
        },
        {
          heading: "3. Legal Basis for Processing Your Data",
          content:
            "The legal basis for processing your personal data is:\n- Consent of the data subject, granted by completing and submitting forms on our website.\n- Execution of a contract or pre-contract if you request our services.\n- Compliance with legal obligations applicable to Artaza Inmobiliaria",
        },
        {
          heading: "4. Data Retention Period",
          content:
            "The data will be retained for as long as necessary to fulfill the purpose for which they were collected and to determine any possible liabilities that may arise. In the case of data used for sending commercial communications, they will be retained until the data subject withdraws their consent.",
        },
        {
          heading: "5. Disclosure of Data to Third Parties",
          content:
            "Data will not be disclosed to third parties, except when required by law or necessary for the provision of the contracted service (e.g., notaries, registries, or technology providers acting as data processors).",
        },
        {
          heading: "6. User Rights",
          content:
            "You have the right to:\n- Access your personal data.\n- Request the rectification of inaccurate data.\n- Request the erasure of data when they are no longer necessary.\n- Object to or request the limitation of data processing.\n- Request the portability of your data.\n- Withdraw your consent at any time, without affecting the lawfulness of processing based on consent before its withdrawal.\n\nYou can exercise these rights by sending a request to the postal or email address indicated in section 1, along with a copy of your ID or equivalent document.\nYou also have the right to file a complaint with the Spanish Data Protection Agency (www.aepd.es) if you believe your rights have been violated.",
        },
        {
          heading: "7. Security Measures",
          content:
            "Artaza Inmobiliaria adopts the necessary technical and organizational measures to ensure the security, integrity, and confidentiality of personal data and to prevent their loss, alteration, or unauthorized access.",
        },
        {
          heading: "8. Changes to the Privacy Policy",
          content:
            "We reserve the right to modify this policy to adapt it to legislative or jurisprudential changes. In such cases, we will announce the changes on this page reasonably in advance of their implementation.",
        },
      ],
    },
  };
  